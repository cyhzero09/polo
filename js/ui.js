const UI = {
  ...
  drawGameOver(ctx, w, h, winner) {
    ...
    if (winner) {
      ctx.fillText(`🏆 ${winner.name} 获胜！`, GAME_OFFSET_X + CANVAS_SIZE / 2, HEADER_HEIGHT + CANVAS_SIZE / 2 - 60);

      if (winner.image && winner.image.complete && winner.image.naturalWidth > 0) {
        ctx.save();
        ctx.beginPath();
        ctx.arc(GAME_OFFSET_X + CANVAS_SIZE / 2, HEADER_HEIGHT + CANVAS_SIZE / 2 - 5, 30, 0, Math.PI * 2);
        ctx.clip();
        ctx.drawImage(winner.image, GAME_OFFSET_X + CANVAS_SIZE / 2 - 30, HEADER_HEIGHT + CANVAS_SIZE / 2 - 35, 60, 60);
        ctx.restore();
        ...
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.fillText(winner.name, GAME_OFFSET_X + CANVAS_SIZE / 2, HEADER_HEIGHT + CANVAS_SIZE / 2 - 5);
      }

      ctx.fillStyle = '#4ecca3';
      ctx.font = '16px Arial';
      ctx.textAlign = 'center';
      ctx.fillText(`剩余血量: ${Math.ceil(winner.hp)} / ${winner.maxHp}`, GAME_OFFSET_X + CANVAS_SIZE / 2, HEADER_HEIGHT + CANVAS_SIZE / 2 + 45);
    } else {
      ctx.fillText('平局！', GAME_OFFSET_X + CANVAS_SIZE / 2, HEADER_HEIGHT + CANVAS_SIZE / 2);
    }
